...
for i=1:NRows
    for j=1:NColumns
        for k=1:NOptsPerPanel
            ...
            if isfield(InStruct,['Allowed' FieldNames{m}]) &&...
                ...
                AllowedValues = InStruct.(['Allowed' FieldNames{m}]);
                CurrentValue = InStruct.(FieldNames{m});
                CurrentIndex = find(cellfun(@(x)isequal(x,CurrentValue),AllowedValues));
    ...
    function changed_popup(varargin)
        Index = varargin{3};
        UIElement = varargin{1};
        
        InStruct.(FieldNames{Index}) = UIElement.String{UIElement.Value};
    end
