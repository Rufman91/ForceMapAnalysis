...
%             line([x500 x500], ylim,'Color','k'); % Draws a vertical line      
        ...
        function [fitresult, gof] = createFit1(obj,SegTime,Filter)
            %CREATEFIT1(Z1_FO,VQ_FO)
            %  Create a fit.
            %
            %  Data for 'untitled fit 1' fit:
            %      X Input : obj.SegTime{j}
            %      Y Output: FilterF{i,j}
            %  Output:
            ...
            %  Auto-generated by MATLAB on 21-Apr-2021 11:16:07


            %% Fit: 'untitled fit 1'.
            [xData, yData] = prepareCurveData(SegTime,Filter);
            
            amplitude = obj.AmplitudeF;
            period = (2*pi)/obj.PeriodF;
            fsc = (2*pi)/obj.firstsignchangeF;
            ...
            opts.StartPoint = [amplitude period fsc];
            ...
            %legend( h, 'vq_Fo vs. z1_Fo', 'untitled fit 1', 'Location', 'NorthEast', 'Interpreter', 'none' );
            % Label axes
            %xlabel( 'z1_Fo', 'Interpreter', 'none' );
            %ylabel( 'vq_Fo', 'Interpreter', 'none' );
            %grid on
                ...
                for j=2:lastseg
                    ...
                    if obj.SegFrequency{j} > 0
                        ...
                        if obj.SegFrequency{j} > 0.25
                            ForceTrend{i,j} = detrend(FZShift{i,j},4);
                            %iN = 500;
                            %obj.FilterF{i,j} = filter(ones(1,iN)/iN,1,ForceTrend{i,j});
                            iN = 50;
                            d = ones(1,iN)/iN;
                        ...
                        hold on
                        plot(obj.ZeroCrossTimeF{i,j}, obj.ZeroCrossF{i,j}, 'bp')
                        hold on
