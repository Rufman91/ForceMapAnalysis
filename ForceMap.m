classdef ForceMap < matlab.mixin.Copyable & matlab.mixin.SetGet & handle & AFMBaseClass
    ...
    properties
        % Properties shared for the whole Force Map. All data is given in SI
    ...
    methods
        ...
        function obj = ForceMap(MapFullFile,DataFolder,TempID,BigData,FakeOpt,NSynthCurves)
            % Author: Manuel Rufin 
            % This is the constructor method of the class. The function
            % specifies the folder where the files are located and imports
            % them. Further it gets the curent folder and returns to it 
            % after the file import. Additionally, it assigns the 
            % properties that can be found in the jpk-file already.
...
%                         end
    ...
    methods
        ...
        function fc_measurement_prop(obj)
            ...
            % Description:
            % This function reads out measurement parameters that are part of the file name of the corresponding jpk data file (i.e. "JPK-FORCE-MAP").
            ...
            elseif isempty(ext42)==0
                ...
                %obj.Linker='amino'; % For only amino functionalized tips
                %obj.Linker='naked'; % For non-functionalized tips
                %obj.Linker='long';
                %obj.Linker='short';
            end
        ...
        function fc_sensitivity_correction(obj)
            % Author: Andreas Rohatschek
            % Description:
            % This function corrects the measured sensitivity before the
        ...
        function fc_sinoidal_fit(obj)
            % Author: Andreas Rohatschek
            % Description:
            % This function fits the data to correct for potential artefacts. Two different fits - sinoidal and linear - are performed and the better fit is applied on the data.
        ...
        function fc_linear_fit(obj)
            % Author: Andreas Rohatschek
            % Description:
            % This function fits the data to correct for a potential tilt by performing a linear fit on the data.
        ...
        function fc_TipHeight_calculation(obj)
            % Author: Andreas Rohatschek
            % Description:
            % This function determines the TipHeight based on the Head Height
        ...
        function fc_estimate_cp_hardsurface(obj)
            % Author: Andreas Rohatschek
            % Description:
            % This function estimates the contact point for force curves measured on hard
            % surfaces (e.g. mica)
        ...
        function fc_xLimit_idx(obj,xLimit1,xLimit2,xLimit3)
            % Author: Andreas Rohatschek
            % Description:
            % This function finds the corresponding indices of an array of data points
        ...
        function fc_selection_threshold(obj,AppThreshValue,RetThreshValue)
            ...
            % force curves based on prerequirements (flags)
            %
        ...
        function fc_snap_in_length_MAD(obj,SelectedFlagStatus)
            % Author: Andreas Rohatschek
            % Description:
            % This function determines the snap-in length using a moving
        ...
        function fc_pulling_length_MAD(obj,SelectedFlagStatus)
            % Author: Andreas Rohatschek
            % Description:
            % This function determines the pulling length using a moving
            % median absolute deviation (MAD)
            % Required input variables:
            % SelectedFlagStatus ... Selected flag status indicates if only 'SMFS_visual_selection_pre' or also 'SMFS_visual_selection_post' have been executed: string, either 'pre' or 'post'
            % Input variable options
        ...
        function fc_adh_force_max(obj,SelectedFlagStatus)
            % Author: Andreas Rohatschek
            % Description:
            % This function finds the maximum adhesion force value in the
        ...
        function fc_adhesion_energy_idxlength(obj,SelectedFlagStatus)
            % Author: Andreas Rohatschek
            % Description:
            % This function determines the adhesion energy by using the previous defined pulling length index
            % Required input variables:
            % SelectedFlagStatus ... Selected flag status indicates if only 'SMFS_visual_selection_pre' or also 'SMFS_visual_selection_post' have been executed: string, either 'pre' or 'post'
            % Input variable options
            % pre ... 'SMFS_visual_selection_pre' has been executed
        ...
        function fc_visual_selection_flag_pre(obj,XMin,XMax,YMin,YMax,NumFcMax)
            % Author: Andreas Rohatschek
            % Description:
        ...
        function fc_visual_selection_post(obj,XMin,XMax,YMin,YMax,NumFcMax)
            % Author: Andreas Rohatschek
            % Description:
        ...
        function fc_print_raw(obj,XMin,XMax,YMin,YMax)
            % Author: Andreas Rohatschek
            % Description:
        ...
        function fc_print_fitted(obj,XMin,XMax,YMin,YMax,Flags)
            % Author: Andreas Rohatschek
            % Description:
        ...
        function fc_print_analysed(obj,XMin,XMax,YMin,YMax,NumFcMax)
            % Author: Andreas Rohatschek
            % Description:
        ...
        function fc_fine_figure_publication(obj,XMin,XMax,YMin,YMax,FmoI,FcoI,Linker,CArea,Axis)
            % Author: Andreas Rohatschek
            % Description:
            ...
            figname=char(figname);
            %% Allocate data
            xApp=(obj.THApp{FcoI}-obj.CP_HardSurface(FcoI))/-Xmultiplier; % Retraction x-data (m): Vertical tip height data corrected by the determined contact point using the hard surface method
            xRet=(obj.THRet{FcoI}-obj.CP_HardSurface(FcoI))/-Xmultiplier; % Retraction x-data (m): Vertical tip height data corrected by the determined contact point using the hard surface method
            yApp=obj.BasedApp{FcoI}/Ymultiplier;
            yRet=obj.BasedRet{FcoI}/Ymultiplier;
            while  obj.PullingLengthIdx
                if strcmpi(CArea,'yes') && nnz(obj.PullingLengthIdx(FcoI))
                    yRetLim=obj.yRetLim{FcoI}/Ymultiplier;
                else
                end
            end
            while obj.SnapInIdx
                if strcmpi(CArea,'yes') && nnz(obj.SnapInIdx(FcoI))
                    yAppLim=obj.yAppLim{FcoI}/Ymultiplier;
                else
                end
            end
            LengthLong1=378;
            ...
            while obj.PullingLengthIdx
                if strcmpi(CArea,'yes') && nnz(obj.PullingLengthIdx(FcoI))
                    area(xRet(1:obj.PullingLengthIdx(FcoI)),yRetLim(1:obj.PullingLengthIdx(FcoI)),'FaceColor',CS5)
                else
                end
            end
            while obj.SnapInIdx
                if strcmpi(CArea,'yes') && nnz(obj.SnapInIdx(FcoI))
                    area(xApp(obj.SnapInIdx(FcoI):end),yAppLim(obj.SnapInIdx(FcoI):end),'FaceColor',CS7)
                else
                end
            end
            plot(xApp,yApp,'Color',RGB1,'LineWidth',10);
            plot(xRet,yRet,'Color',RGB2,'LineWidth',10);
        ...
        function [CS1,CS2,CS3,CS4,CS5,CS6,CS7,CS8,CS9,CS10]=fc_ColorScheme(obj)
            % Author: Andreas Rohatschek
            % Description:
            % This function provides a color scheme created using colorbrewer2.org
...
%             end
        ...
        function [HeadHeight ,Force,spring_constant,sensitivity]=...
            ...
            % Author: Orestis Andriotis, Manuel Rufin
            % Description:
            % This function reads out the height (measured and deflection) as well as scaling factors and gives them as output. Important directories found are: 
...
%                     imshow(mask)
        ...
        function unpack_jpk_force_map(obj,MapFullFile,DataFolder)
            % Author: Manuel Rufin
            % Description:
            % This function unpacks the jpk-file into a temproary folder
            % and reads out important parameters.
        ...
        function read_in_header_properties(obj)
            % Author: Manuel Rufin
            % Description:
            % This function verifies the jpk-software version and reads out important force map properties.
        ...
        function load_force_curves(obj)
            % Author: Manuel Rufin
            % Description:
            % This function reads out the vertical deflection data and the 
            % capacitive sensor height data from the jpk-file and allocates
            % them to the force map properties.
