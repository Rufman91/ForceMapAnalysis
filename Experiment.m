...
%                 print(gcf,fullname,'-dpng');
    ...
    methods
        ...
        function show_image(obj)
            ...
            h.ColorMode(1).Backdrop = [.05 .05 .05];
            h.ColorMode(1).Buttons = [.1 .1 .1];
            h.ColorMode(1).ButtonText = 'w';
            ...
            h.ColorMode(2).Backdrop = [.95 .95 .95];
            h.ColorMode(2).Buttons = [.9 .9 .9];
            h.ColorMode(2).ButtonText = 'k';
            ...
            ToggledMainMenuIndizes = [18 19 20 21 22 31 1];
            ToggledVolumeMenuIndizes = [1 2 3 4 5 6 7 8 9];
            ToggledResultMenuIndizes = [1];
...
%                         'FontSize',obj.ShowImageSettings.ReferenceFontSize);
            ...
            function update_interface(varargin)
                ...
                for i=1:length(ToggledMainMenuIndizes)
                    h.B(ToggledMainMenuIndizes(i)).Visible =...
                        obj.ShowImageSettings.MainMenuValue;
                end
                for i=1:length(ToggledVolumeMenuIndizes)
                    h.FVM(ToggledVolumeMenuIndizes(i)).Visible =...
                        obj.ShowImageSettings.VolumeMenuValue;
                end
                for i=1:length(ToggledResultMenuIndizes)
                    h.Res(ToggledResultMenuIndizes(i)).Visible =...
                        obj.ShowImageSettings.ResultsMenuValue;
                end
            end
            ...
            function get_and_draw_volume_information(varargin)
                ...
                h.VolumeStruct.ClassType = class(Class{TempIndex});
            ...
            function draw_volume_information(varargin)
                ...
                h.VolumeStruct.ClassType = class(Class{h.VolumeStruct.Index});
                if ~isequal(h.VolumeStruct.ClassType,'ForceMap')
                    warning('This Class Instance does not have Volume information!')
                    try
                        delete(h.ImAx(3));
                ...
                IsLoaded = Class{h.VolumeStruct.Index}.check_if_zipped_file_is_loaded(1);
                ...
                h.VolumeStruct.ListIndex = ...
                    Class{h.VolumeStruct.Index}.Map2List(...
                ...
                % options
                [RawApp,RawHHApp] = ...
                    Class{h.VolumeStruct.Index}.get_force_curve_data(...
                    h.VolumeStruct.ListIndex,0,0,0);
                [RawRet,RawHHRet] = ...
                    Class{h.VolumeStruct.Index}.get_force_curve_data(...
                    h.VolumeStruct.ListIndex,1,0,0);
                if obj.ShowImageSettings.UseCorrectedSensitivity &&...
                        ~isempty(Class{h.VolumeStruct.Index}.RefSlopeCorrectedSensitivity)
                    CorrectedSens = ...
                        Class{h.VolumeStruct.Index}.RefSlopeCorrectedSensitivity;
                else
                    CorrectedSens = Class{h.VolumeStruct.Index}.Sensitivity;
                end
                Sens = Class{h.VolumeStruct.Index}.Sensitivity;
                SpringConstant = Class{h.VolumeStruct.Index}.SpringConstant;
                if obj.ShowImageSettings.BaselineCorrected &&...
                        Class{h.VolumeStruct.Index}.BaseAndTiltFlag
                ...
                if obj.ShowImageSettings.ContactPointShifted
                    CP = Class{h.VolumeStruct.Index}.CP(h.VolumeStruct.ListIndex,:);
                else
                    CP = [0 0];
                end
